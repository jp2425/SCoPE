...
class CPP14ParserListener(ParseTreeListener):
    ...
    def __init__(self):
        self.inside_enum_specifier = False
    ...
    #Esta função de visita em profundidade pode ser otimizada.
    #Como percorremos a árvore da direita para a esquerda, se o símbolo terminal mais à diretia não for um igual
    # Já sabemos que não é uma atribuição mas sim o erro da gramática.
    def visit(self, child):
        from grammars.CPP14Parser import CPP14Parser

        if type(child) == CPP14Parser.BraceOrEqualInitializerContext:
            return [child.getText()[0]]


        elif type(child) == TerminalNodeImpl:
            if child.getText() == ',':
                return []
           # print("Terminal: ", child.getText())
            return [child.getText()]
        elif child.children:
            tokens = []
            for child2 in reversed(child.children):
                a =  self.visit(child2)
                if len(a) > 0:
                    tokens = tokens + a
            return tokens
    ...
    def enterSimpleDeclaration(self, ctx:CPP14Parser.SimpleDeclarationContext):
        ...
        if init_declarator_list:
            equalSign = 0
            tokens = []
            for child in reversed(init_declarator_list.children):

                #print("Variável definida pelo programador:", child)
                tk = self.visit(child)
                if tk:
                  tokens = tokens + tk
            if tokens[0] == "=":
                tokens.pop(0)
                print(tokens)
    # Resto do código para análise sintática e invocação do listener
    ...
    def enterStorageClassSpecifier(self, ctx:CPP14Parser.StorageClassSpecifierContext):
        ...
        print("Classe definida pelo programador:", class_name)
        pass
    ...
    def enterEnumHead(self, ctx:CPP14Parser.EnumHeadContext):
        print("Enum definido pelo programador ", ctx.getText())
        pass
    ...
    def enterFunctionDefinition(self, ctx:CPP14Parser.FunctionDefinitionContext):
        function_declarator = ctx.declarator().pointerDeclarator().noPointerDeclarator()
        function_name = function_declarator.noPointerDeclarator().getText()
        print("Função definida pelo programador:", function_name.replace(")","").replace("(",""))
        #params = function_declarator.parametersAndQualifiers().parameterDeclarationClause().parameterDeclarationList().getText()
        #print("Params:", params)
